...
// 行わない（U9）。
//
// **この門番は上の脅威を塞がない**（2026-09-07 のセルフレビューで実測）。攻撃者は
// 無支払いの `tools/call` で 402 応答から `accepts` をタダで手に入れ、それを `accepted` に
// 転記し、`authorization` を条件どおりに手書きし、署名欄に任意の 130 桁 16 進数を
// 入れるだけでここを通過できる。upfront では上流が /verify を呼ばないため、署名が
// 実際に検証されるのは facilitator の /settle だけで、その手前にある唯一のゲートが
// 「130 桁の 16 進数か」という形式検査になる。つまりここが弾けるのは「攻撃者が
// わざわざ選ぶ理由の無い壊れ方」だけで、位置づけは軽量な入力検証にとどまる。
// 実際に塞ぐには U9 の署名復元が要る。x402-flow.test.ts の「捏造ペイロード」の
// テストが、この限界を実行可能な形で残している
//
// 要求の照合を scheme / network の 2 つだけで行うのは、上流が accepted ブロック全体の
// 一致をこの門番の後段で見るため。ここで厳しくしても得るものが無く、上流の照合規則が
// 変わったときに正当な支払いを弾く側へ倒れる。一方で、マッチした要求には eip3009 の
// authorization を必ず要求している。この売り手が広告する条件が exact / eip3009 の
// 1 通りだけだからで、accepts に別のスキームを足すときはここも直す必要がある
