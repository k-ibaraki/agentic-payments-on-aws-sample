...
// （2026-09-21 ユーザー判断）。実運用で価格帯ごとの粗利差を大きく取るなら、署名を足す。
import type { TierTable } from "./tiers.js";

/** 価格帯（DESIGN.md 決定56）。表示名は梅・竹・松だが、見積書や JSON には ASCII の鍵で載せる */
 ...
 *
 * 外部入力なので例外は投げない。壊れた形も知らない価格帯も「読めない」の一語に畳む。
 * 読めなければ呼び出し側が判定し直す
 */
 ...
 *
 * 価格帯に対する価格が表と食い違う見積書は使わない。価格表が差し替わった直後の古い
