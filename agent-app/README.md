...
- ブラウザ UI は `index.html` + `src/index.ts`（認証・チャット・ウォレット・購入履歴）。構成:
  ヘッダーの 1 行に題名・タブ 3 枚・利用者を並べる（依頼タブは会話、ウォレットタブは残高と支払いの枠、購入履歴タブは会話をまたいだ一覧と右のプレビューを表示する）。
  依頼タブは画面の高さに収め、会話欄の中だけをスクロールさせる（入力欄は常に画面の下端）。
  ...
  購入履歴タブの一覧は「表示」を押したものだけを載せる。会話の中のカードは、テキストの返答と同じ幅で、
  高さは既定で会話欄の半分。「拡大」を押すと画面いっぱいのモーダルになり、「閉じる」か Esc キーで戻る
...
- 応答と途中経過を受ける WebSocket（API Gateway）は、接続から 2 時間で必ず切られる。放置したタブでは、
  ブラウザがタイマーを間引くため生存確認が遅れ、10 分の無通信で切られることもある。AWS Blocks のクラウド用の購読は
  切れても繋ぎ直さないので、画面の側で切断を拾って購読し直す。待っている応答が無ければ、次に送るときに購読し直す
  （画面はそのまま）。応答の途中か承認待ちで切れたら、その場で会話を読み直す。このとき、その回の途中経過は
  「受信が途中で切れた」で打ち切られ、会話の中の購入カードは会話の末尾へ移る（画面を読み込み直したときと同じ）。
  判断の規則は `src/ui-rules.ts`
...
- 依頼タブの上端の行に、ウォレット残高と支払いの枠の残枠を表示する（値が変わった瞬間に光る）。支払った瞬間が分かるよう、
