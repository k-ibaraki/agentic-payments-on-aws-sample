...
| 18 | フェーズ②の検証手段 | TDD は facilitator を偽 HTTP サーバーで差し替えて進め、仕上げに使い捨てウォレット（viem 鍵 + Circle Faucet のテスト USDC + x402.org facilitator）で実オンチェーン決済を 1 回流す。価格は 0.1 テスト USDC / 呼び出し（exact スキーム） | テストの安定性と実決済の証拠取りを両立。買い手本命（AgentCore Payments）の検証はフェーズ③以降。価格は当初 0.01 テスト USDC で仮決めしていたが、クラウドへの初回デプロイ時にユーザー判断で 0.1 へ引き上げた（2026-08-31 更新） |
