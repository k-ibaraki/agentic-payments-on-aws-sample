...
## 2026-09-26: PR #40 のレビュー指摘への対応（受信が切れたときの立て直しの取りこぼし、決定69 追記）

Jev を使った PR レビュー（PR には投稿せず、会話に出した）の指摘 6 件のうち、ユーザーが 4 件（1・2・3・5）を直すと決めた。
残りの 2 件（Issue の紐付けが無い、`subscribe` のラッパーの中で繋ぎ直す案）は直さない。

### やったこと

- 指摘1（承認待ちで切れると、その回の途中経過が「進行中・N 秒」のまま止まる）: 途中経過を打ち切る条件が
  `chat.isLoading()` だけだった。useChat は `interrupt` を受けた時点で loading を戻すので、承認待ちでは打ち切りの行が
  足されず、読み直しの `finishTimeline()` は `endedAt` を入れないため見出しが凍っていた。README と決定69 ③の
  「承認待ちで切れても打ち切る」と食い違っていた。進行中の途中経過があれば状態を問わず打ち切る形にし、
  `applyDisconnect` は終わった経過に何も足さないようにした
- 指摘2（応答の途中で読み直した後にもう一度切れると、送信が止まったまま戻らない）: 読み直した後の新しいフックは
  loading を持たないので、2 度目の切断で `planRecovery` が「購読を外すだけ」を選んでいた。送信を止めている印
  （`awaitingRecoveredReply`）を戻す終わりの知らせは、購読が無いので届かず、画面の再読み込みまで戻らなかった。
  `planRecovery` に `awaitingRecoveredReply` を渡し、立っていればその場で読み直す側に回した
- 指摘3: `Timeline.endedAt` のコメントに、受信が切れて打ち切ったときも入ることを足した
- 指摘5（読み直しを待つ間に Enter を続けて押すと、空の途中経過が 1 つ余分に出る）: 入力を消すのが接続の確かめの後に
  なったため、後の Enter も同じ文を持って進んでいた。二度目の送信そのものは useChat の loading で捨てられるが、
  `beginTimeline()` は二度走っていた。確かめている最中の Enter は捨てるようにした（`checkingBeforeSend`）
- テストを先に書いた（`timeline.test.ts` 2 件、`ui-rules.test.ts` 1 件。赤 2 件を確認してから実装。残る 1 件の
  「承認待ちで切れても打ち切る」は今の `applyDisconnect` の振る舞いを固めるもの）

### 確かめたこと

- `npm run test`（311 件）と `npm run typecheck`
- index.ts の配線（打ち切りの条件と Enter の歯止め）は単体テストの外で、ブラウザでは確かめていない

### つまずき

- レビューの過程で、設計の観点から「承認ボタンの二重押しで二重承認になる」「送信の二重押しで二重購入になる」と
  指摘されたが、どちらも実害は無かった。読み直しは最初の `await` の前に承認の欄を空けるので 2 度目のクリックは
  外れたボタンに当たり、useChat の `sendMessage` / `respondToInterrupt` も loading で二度目を捨てる。残ったのは
  指摘5 の見た目の乱れだけだった
- テストで `startTimeline` が最初の 1 行（「依頼を送りました」）を持つことを見落とし、期待する件数を 1 つ少なく書いた

## 2026-09-26: 放置すると応答の受信が切れて届かなくなる不具合を、画面の側で立て直す（決定69）

- ユーザーの報告: 買い手の画面を開いたまましばらく放置すると、ストリーミングを受け取れなくなる。裏では動いているようだ
- 調査: 応答と途中経過は 1 本の WebSocket（API Gateway）で受けている。API Gateway はこの接続を 2 時間で必ず切り、
  10 分の無通信でも切る。クライアントは 9 分ごとに生存確認を送るが、放置したタブではタイマーが間引かれて
  間に合わないことがある。クラウド用の購読（`bb-realtime` の aws-middleware）は切れても繋ぎ直さない。
  useChat（`bb-agent`）は購読済みの印（`activeSub`）を持ち続けるので、切れていても購読し直さずに送信する。
  その結果、エージェントは動くのに画面には何も届かず、`done` が来ないため送信ボタンも戻らなかった。
  ローカル用の mock-middleware は自動で繋ぎ直して購読を再送するため、手元では再現しなかった
- 方針はユーザーと相談して決めた: まずアプリ側で回避する（今回）。ライブラリ側の修正と上流への issue は後で調べる
- 実装: 判断の規則（`planRecovery`・`isReplyFinished`）を `ui-rules.ts` に、途中経過の打ち切り（`applyDisconnect`）を
  `timeline.ts` に切り出し、テストを先に書いた（レッド 8 件 → グリーン）。`index.ts` では購読を `onDisconnect`
  付きの形に改めた。待っている応答が無ければ `destroy()` で購読だけを外し、応答の途中か承認待ちならフックを
  作り直して会話を読み直す。送る前・承認へ応答する前・`online`・`visibilitychange` にも接続を確かめる
- 確認: ローカルで Playwright を使い、一度応答を受けた後に Blocks サーバーとの WebSocket をブラウザ側から閉じ、
  もう一度依頼を送った。mock は自動で繋ぎ直すため、そのままではクラウドの不具合にならない。そこで、
  再接続の `onopen` の中で送られる購読の再送だけを捨てる細工を差し込んだ。こうすると、接続は戻っても
  切れる前の購読は戻らず、クラウドで購読が失われたときと同じ状態になる。
  修正前の `index.ts` では 2 回目の応答が空の吹き出しのまま届かず、送信ボタンも押せないまま残った（報告の症状が再現）。
  修正後は切断を 1 回だけ検知し、2 回目の応答も届いた（`done` は 2 件で、重複は無い）。売り手の URL は到達できない
  宛先に向け、モデルはローカルの mock（定型の応答）だったので、実決済もモデルの呼び出しも起きていない
- 確かめていないこと: 応答の途中で切れた場合の読み直し（mock の応答は一瞬で終わるため作れなかった）、オフラインでの
  読み直しの失敗とやり直し、デプロイ環境での 2 時間の切断
- `npm run test`（301 件）と `npm run typecheck` が通った
- セルフレビュー（1 巡目）で直したこと: 読み直しの最中に応答が終わると、その読み直しは応答を保存する前の履歴を
  読んだかもしれないのに、終わりの知らせは使い切られていた。「まだ応答中」と判定されたまま送信が戻らなくなり得る
  （推測。再現はしていない）。読み直しの最中に終わりの知らせが来たら、その後にもう一度読み直すようにした。
  あわせて、途中経過の要約の変わり方を決定65 に追記し、`timelineSummary` と `createChat` のコメントを今の動きに合わせた
- セルフレビュー（2 巡目）で直したこと: 会話を捨てた後に古い読み直しが終わると、その後に始めた読み直しの印まで
  消していた（推測。新規会話の直後にまた受信が切れたとき、読み直しが二重に走り得る）。読み直しを 1 本ずつ走らせる
  仕組みを `createSingleFlight`（`ui-rules.ts`）に切り出し、自分の番の処理だけが印を消すようにしてテストを足した。
  テストが要所を押さえているかは、実装のガードを一つずつ外して確かめた。捨てた処理が新しい処理の「もう一度」を
  横取りする場面は最初のテストでは捕まらなかったので、その場面のテストを足した。`npm run test`（308 件）と
  `npm run typecheck` が通った
- セルフレビュー（3 巡目）で直したこと: `agent-app/README.md` のアーキテクチャに、受信が切れたときの振る舞い
  （接続の上限、画面の側で購読し直すこと、読み直すと購入カードが末尾へ移ること）を書き足した。決定の番号は書いていない
- セルフレビュー（4 巡目）で直したこと: useChat の `loadConversation` は承認待ちが残っているときにしか承認の欄を
  描き直さないため、読み直しても前の承認のボタンが残っていた。切れている間に別のタブで承認に応えると、残ったボタンは
  サーバーに弾かれ、承認待ちの判定も誤って応答の途中の送信の停止が外れ得た（推測。再現はしていない）。読み直す前に
  承認の欄を空け、読めなかったときは元に戻すようにした
- セルフレビュー（5 巡目）で直したこと: 承認のボタンを押したときに受信が切れていると、読み直しで欄が描き直された後も
  押した時点の ID で応答を送っていた。その承認が既に済んでいるとサーバーに弾かれ、useChat の応答中の印が残って
  送信が戻らなくなり得た（推測。再現はしていない）。読み直しで押したボタンが画面から外れていたら送らず、描き直された
  欄から選び直してもらうようにした
- セルフレビュー（6 巡目）は指摘なし。直しを重ねたので、最終形でもローカルの確認（受信を切ってからもう一度送る）を
  やり直し、2 回目の応答が届くこと、ページのエラーが出ないことを確かめた。`npm run test`（308 件）と
  `npm run typecheck` が通った。確認に使ったテスト用の利用者は、その会話ごと手元のデータから消した
- PR #40 を出した後、上流の AWS Blocks を調べた（決定69 の追記）。公開の issue は無く、同じ原因を扱う PR が 3 本
  （#497 マージ済み、#501・#503 未マージ）あった。npm の最新版の中身も確かめたが、繋ぎ直しは入っていなかった
  （#497 のマージは最新版の公開より後）。#497 だけでは 2 時間の上限の後の繋ぎ直しが古いトークンで弾かれるため、
  放置の場合は #503 まで要る。ユーザーと相談し、上流の修正が公開されるまではこの回避策で運用し、上げるときに
  回避策を見直すことにした

## 2026-09-26: 会話の中の購入カードを吹き出しと同じ幅にし、拡大を画面いっぱいのモーダルにする（決定68）

- ユーザーが Amplify 環境で決定67 の画面を試し、iframe を変えたいと申し出た。通常時は横幅もテキストの返答と
  同じに狭め、拡大時はブラウザ全体近くまで広がるモーダルにして、閉じるボタンで戻したい
- 下調べ: 会話欄は決定67 で `container-type: size` にしてあり、これはレイアウト封じ込めを伴う。そのため、中のカードを
  `position: fixed` にしても基準は会話欄になり、画面いっぱいには広がらない。そこで Popover API を試した。静的な試作で、
  Chromium と WebKit のどちらでも、カードを DOM の上で動かさずに最前面の層へ出せ、iframe の読み込み直しも
  起きないことを確かめてから方針を出した。閉じ方（ボタンと Esc）と出し方（別の PR）はユーザーが選んだ
- 実装: 通常時は `.purchase-card` を `width: 85%`・左寄せにした（吹き出しの最大幅と同じ）。拡大ボタンでカードに
  `popover="manual"` を付けて開閉し、Esc は `document` の `keydown` で閉じる。決定67 の「会話欄いっぱいに広げて
  スクロールを合わせる」仕組み（`.expanded`・`100cqh`・`scroll-margin-block`・`scrollIntoView`）は外した。
  ボタンの文言は `previewExpandButton` の期待値から先に直した（レッド → グリーン）
- 確認: 今回は本物の拡大ボタンの処理を通した。売り手の代わりに、MCP の `resources/read` に ext-apps の View
  （`app-with-deps.js` を読み込むだけの小さな HTML）を返す偽の売り手を手元に立て（課金は持たない。コミットしていない）、
  `BILLING_MCP_URL` をそこへ向けた。購入の一覧（`buyer.listPurchases`）と成果物（`buyer.getPurchasedHtml`）だけを
  Playwright でブラウザ側で差し替えた。`PAYMENT_*` を渡していないので、実決済が起きる経路は無い。
  結果: 1280×720 で 1200×640、390×844 で 359×813 まで広がり、Esc と「閉じる」で元の位置と大きさに戻る。
  View の文書の起動時刻と表示回数が開閉の前後で変わらず、読み込み直しは起きていない。Chromium と WebKit で同じ
- つまずき: 開いた姿の高さが中身の分（236px）にとどまった。通常時の左寄せに付けた `align-self: flex-start` が、
  固定配置の箱にも当たって縦に縮めていた。開いた姿で `align-self: stretch` に戻した。もう一つ、閉じるとカードが
  58px 下にずれた。開いている間はカードが流れから抜けて会話欄のスクロール位置が詰められるため。開く直前の値を
  `beforetoggle` で覚え、閉じたときに戻すようにした
- 残る制約: Popover はモーダルではないので、開いている間も Tab でフォーカスが背後へ移り得る。フォーカスが iframe の
  中にあるときは Esc が届かず、「閉じる」ボタンで閉じる
- `npm run test`（293 件）と `npm run typecheck` が通った。deploy と実決済での確認はしていない
- PR の前の見直しとセルフレビューで直したこと: 拡大ボタンの注記が、会話の破棄でカードが DOM から外れたときにも
  `toggle` が届くかのように読めた（実際はブラウザが閉じるだけで、`toggle` は届かない）。決定50 の行の追記
  （決定67 の「会話欄いっぱい」）が古くなったので、決定68 の追記を足した。TypeScript の型定義で最初から
  `ToggleEvent` になっているイベントに付けていた型の書き換えを外した

## 2026-09-26: 依頼の面を画面の高さに収め、会話の中の購入ページに拡大ボタンを付ける（決定67）

- ユーザーの依頼（会話欄をできるだけ大きく、ヘッダーを小さく、小さい画面でもはみ出さないように、iframe を縦に広げて
  拡大ボタンを付けたい）を grill-me で詰めた。「ヘッダー」は会話欄より上の全部を指すこと、画面の高さに収めること、
  残高と残枠は決定47 の狙いを保って小さくすることなどを一つずつ決めた（決定67 の理由欄）
- 下調べで分かったこと: 会話欄の上には、題名の帯・タブ・区画の見出し・残高の帯・説明文が積まれていた。会話欄は
  `max-height: 70vh` で画面に収まらず、ページと会話欄のスクロールが二重になっていた。売り手の View は
  `body { height: 100vh }` で枠いっぱいに広がる作りなので、MCP Apps の size-changed は役に立たず、高さはホストが決める
- 実装: ヘッダーと `main` を `.app-shell` で包み、依頼の面のときだけ `100dvh` の flex の列にした。タブをヘッダーの
  行へ上げ、区画の見出しを読み上げ用に隠し、その行に残高・残枠と「新規会話」を並べた。説明文は会話が空のときだけ
  重ねて出す。会話欄を `container-type: size` にし、購入カードの iframe を `max(240px, 50cqh)` にして、拡大ボタンで
  カードを `100cqh` に広げる。ボタンの文言は `ui-rules.ts` の `previewExpandButton` に切り出し、テストを先に書いた
- 確認は決済なしで行った。ローカル（`BUYER_SELF_SIGNUP=true BUYER_LOCAL_MODEL=canned`）に架空のメールで
  サインアップし、Playwright で会話欄にダミーの購入カード（静的な iframe）を差し込み、拡大の前後を撮った。
  実際の売り手 View と、`index.ts` の `attachExpandButton` そのもの（ボタンを付ける処理）は通していない。
  差し込んだカードには同じクラスとトグルを持たせた
- 実測（内部情報パネルを開いた状態）: 1280×720 で、会話欄の上端が 464px → 152px、画面内の会話欄が 256px → 464px。
  ページのはみ出しは 212px → 0。1366×768 で 512px、1440×900 で 644px、1920×1080 で 812px
- つまずき: 最初は 1280px 幅でヘッダーが 2 行に折り返した。題名 219 + タブ 313 + 利用者 326 + 隙間が、ナビの幅 840px
  を超えていた。題名を略す境目を 62rem で書いたが、Pico は 1280px 以上で基準の文字の大きさを 20px に上げるので
  1240px 相当になり、本文の上限 1280px の中では略さない題名が一度も出なかった。測り直すと、「サインイン中:」の前置きを
  付けたままでは 1920×1080 でも 1354px 要り、どの幅でも 1 行に収まらない。そこで前置きを外して名前は `title` で見せ、
  境目を 60rem、細いときの名前を 8em にした。これで 1280〜1920px のどれでも 1 行（69〜71px）に収まる
- 拡大したカードは、会話欄の `flex-shrink: 1` のままだと中身が溢れたときに縮められる。子の `flex-shrink` を 0 にした。
  上端に合わせたときに会話欄の内側の余白ぶん潜るので、カードに `scroll-margin-block` を付けた
- 購入履歴の二段組は、タブの行が無くなった分だけ差し引きを 20rem から 16rem に詰めた。1280×720 と 1366×768 は
  下限の 24rem に当たり、これまでどおり少しはみ出す（ページのスクロールで届く）
- 確認: `npm run test`（26 ファイル・293 件）と `npm run typecheck` が通った。スマートフォンの幅（390×844）は
  対象外だが、崩れていないことだけを見た。deploy と実決済での確認はしていない
- 利用者名の前の「サインイン中:」を外したのは、grill-me で決めた項目に無く、実測から実装中に決めたこと。報告の冒頭で
  ユーザーに伝えた
- セルフレビューで直したこと: タブをヘッダーへ移すときに `.tabs` へ `box-shadow: none` を足し、キーボードのフォーカスが
  見えなくなっていた。Pico は `role="group"` の中のボタンの輪を消し、グループ全体の `box-shadow` で輪を描くため。
  タブごとに `:focus-visible` で輪を描くようにし、実機で確かめた（Pico のボタンには `box-shadow` の遷移があり、
  直後に測ると透明に見えた）。Pico が 2 枚目以降のタブに当てる -1px の左余白も打ち消した。ウォレットタブの案内文に
  残っていた「帯の残枠」は「依頼タブの残枠」に改めた。コードの中の「帯」は `.wallet-strip` の呼び名として残し、
  `ui-rules.ts` の見出しで定義した

## 2026-09-26: 1回の支払い上限を画面から変えられるようにする（決定66）

- ユーザーの依頼を grill-me で詰めた。目的は「買い手が 1 回あたりの歯止めを自分で決める体験を見せる」ことで、
  松を通すこと自体ではない。持ち方は利用者ごと、天井なし、USD の小数 2 桁で入力、セッションは破棄しない、
  売り手と LLM には伝えず閾値だけを変える、画面は支出上限と別の行と入力欄、既定に戻す操作は付けない、の順に決めた
- 下調べで分かったこと: 1 回の上限は `x402-payer.ts` の `selectAcceptable` がアプリの中で判定しているだけで、
  AgentCore Payments とは関わらない。支出上限（決定43）がセッションの破棄を要したのは上限が作成時に固定されるため
  で、1 回の上限にはその事情が無い。破棄すると残枠が立て直って枠が実質広がるだけなので、破棄しない側を採った
...
- 画面の入力欄は既存の `.spend-limit-input` の見た目を共用するため、クラス名を `.limit-input` に改め、
  狭い画面の規則も ID ではなく `input` で当てるようにした。サインアウト時に消す行の一覧にも足した
- 画面の注記は支出上限と逆のことを書いた（支出上限は「変えるとセッションを破棄」、1 回の上限は「セッションはそのまま、
...
- 前日の最新化で、構成図に「決定65」を 3 か所、ルート README に 1 か所、agent-app/README.md に「決定26 の訂正」を
  書き込んだ。README と構成図には決定番号を書かず、そのファイルだけで意味が通るように書く取り決め（決定32・38）に反する
- 洗い出すと、前日の分だけでなく、取り決めの後の作業で少しずつ入り込んだ参照が残っていた。PR #35 の分
  （agent-app/README.md 2 か所・billing-mcp/README.md 1 か所）と、それ以前の分（agent-app/README.md 9 行・
  billing-mcp/README.md 7 行・ルート README 1 か所。billing-mcp/README.md の未決論点の番号 1 か所を含む）
- ユーザーの指示ですべて外した。どの箇所も理由は本文に書いてあり、番号を外すだけで意味が通った。外の文書へ誘うだけの
  「詳細は DESIGN.md 決定58」は文ごと削った。ルート README の「この README で触れている『決定N』はここを指します」は
  指す先が無くなったので、決定録の中身を案内する文に改めた
- 入り込み続けた原因: 取り決めが DESIGN.md の理由欄にしか無く、作業のたびに読む CLAUDE.md に無かった。
  しかも CLAUDE.md の「必ず守ること」は「設計判断に番号を振る」ことだけを求めており、番号をどこに書いてはいけないかは
  書かれていなかった
- 再発防止: CLAUDE.md の「必ず守ること」に、番号を使ってよい文書と使わない文書（README・構成図）を並べて書き、
  コミット前に打つ確認の grep を添えた。構成図は PNG に XML が圧縮して埋め込まれていて grep が効かないので、編集に使う
  `.drawio` の段階で探すことも書いた。DESIGN.md・実装記録・CLAUDE.md・AGENTS.md・コードのコメントは、これまでどおり
  決定番号で参照する
- CI で機械的に止める案（README と構成図の埋め込み XML を検査する）も出したが、ユーザーの判断で見送った。
  まずは CLAUDE.md の強化で様子を見る
