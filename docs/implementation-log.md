...
- 事実の陳腐化も1件修正: `agent-app/README.md` が `PAYMENT_MAX_AMOUNT` の既定値を「100000 = 0.1 USDC」と
  書いていたが、決定59 でコードの既定値は `150000`（0.15 USDC）に上がっていた。合わせて「0.1 テスト USDC」と
  固定額で書かれていた2箇所も、決定56 の価格帯制を踏まえた表現に直した
- DESIGN.md は「行を消さず経緯を残す」方針のため、内容の削除・要約はせず日本語の言い回しのみ4件
  （助詞の誤り1件、文が完結しない箇所1件、句点の脱落2件）と、浮いた半角スペース6箇所を直した。
...
- コメントの重複排除は「定義元・詳細な説明を持つファイルに残し、利用側は決定番号または参照先へ圧縮する」
  方針で統一した。セキュリティ上の脅威モデルや実測に基づく非自明な事実（例: `@x402/mcp` が `accepted`
  ブロックしか照合しない、`Runtime.NodeJsExit` の実測）は圧縮の対象にしても削除はしていない
