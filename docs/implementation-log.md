...
- ユーザーの「むしろ判断すべきは売値では」という指摘で、値付けこそが正しい置き場だと分かった。
  ただし判定器としての Jev の優位（0.1 秒・$0.00002）は、$0.1 の取引と 600 秒の処理予算に対して
...
- 割引の規則（トークン単価表）と、上限に積む安全余裕の扱いの設計。安全余裕は `exact` の買い手が
  満額払ううえ、`upto` の買い手でもセッションの枠を上限額で食うため、率に天井を設けるかを決める
