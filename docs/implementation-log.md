...
二重に課金されることはないが、settle 往復そのものは発生し続ける。攻撃者は正規の
支払いを 1 度買う必要があるぶん敷居は上がる。止めるなら payer + nonce の重複弾きが要り、
Lambda がステートレスである以上コンテナ内メモリでは取りこぼす（U9 の範囲外・未対応）。
...
- 弾くときは支払い要求の形ではなく、ただの失敗として返す。支払い要求の形で返すと買い手が
  「まだ払っていない」と解釈してもう一度署名を作り、PaymentSession の枠を二重に消費するため
- テストは先に書いて落とした（TDD）。`payment-guard.test.ts` 17 件（単体）と、
  `x402-flow.test.ts` に統合 5 件（実クライアントに買わせた正規ペイロードを壊し、
  `/settle` が増えないことを確かめる）。server 57 件・CDK 10 件すべて通過
- `billing-mcp-server.test.ts` の `buildPayment` は `authorization: {}` という実クライアントが
  作らない形だったため、eip3009 の形に揃えた

### 見送ったもの（U9・Tier 2 として記録）

- **EIP-712 署名そのもののローカル復元**（U9 に起票）。形の整った偽署名を確実に止められるが、
  スマートコントラクトウォレット（ERC-1271 / 6492）の正当な支払いを弾く恐れがあり、
  本サンプルの買い手が EOA かどうかが未確認。次の実決済で `authorization.from` を実測してから決める
