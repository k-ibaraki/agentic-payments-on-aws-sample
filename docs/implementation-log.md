...
- 売り手は決定56 で三段（梅 $0.1 / 竹 $0.15 / 松 $0.2）になり、402 の `accepts[].amount` には
  最小単位（`150000` 等）が載る
...
決定56 で売り手の価格表が3段（梅 $0.1 / 竹 $0.15 / 松 $0.2）へ変わったのに追随していなかったこと。
...
## 2026-09-21（追記）: 判定器の切り替えをクラウドで実地に確かめた（決定58）
...
| `judge: systemone` ＋ 鍵投入 | ログに確信度が付く（0.85〜1.00）。Jev が判じている |
| `judge: bedrock` へ戻す | 確信度が消える |

段の根拠は `resource.description` に載って買い手へ届いていた
...
- 運用上の含意として、切り替えの最中は同じ依頼が回によって別の判定器に当たり得る。
...
- Jev は同じ入力でも確信度が 0.85〜0.88 と揺れた（段は松のまま）。決定58 ⑥の「境界で揺れる」と整合する
...
- 「両判定器に同じ文言を渡して比べた」と書いたが、その文言はもともと Bedrock 用の
...
- 片方のために書かれたプロンプトで両者を比べない。文言は判定器ごとに効く
- 注記（過学習の疑いなど）を一方にだけ付けない。同じ土俵にあるものには等しく付ける
- 本測定が覆うのは「日本語の依頼文から 3 段の規模を判ずる」1 課題・20 件のみで、
  Jev 一般の性能について述べたものではない

## 2026-09-21: 段の判定器を Haiku と Jev で切り替えられるようにする（決定58）
...
- 本物の Jev（`jev-1.13.0`）を実測した。鍵はユーザーが用意し、`.env` 経由で受け取った
  ...
  現存していたのでそのまま使い、正解は 4 段の主観ラベル。3 段で測るときは
  small+medium→梅 / large→竹 / xlarge→松 と畳んだ
- 判定器の切り替えを実装した。AppConfig の既存 profile（`tier-table`）に `judge` の欄を足し、
  `bedrock`（既定）と `systemone` を選べるようにした。判定器はリクエストごとに選ぶ
...
- API キーを Secrets Manager から読む経路を足した（`pricing/typesafe-key.ts`）。
  CDK は目印入りの器だけを作り、投入は `pnpm set:jev-key`
...
- **「画面」の語が実態と合っていないというユーザー指摘は正しかった。** 売り手が返すのは常に HTML 1 枚で、
  ...
  性能を採って現行据え置きとした。判定器ごとに最適な文言が違うという事実が残った
- **分量の語彙への書き換えは両判定器とも悪化した。** 段は分量（目安トークン数）を決めるものなので
  筋は通っているように見えたが、実測では負けた。決定56 の「出力量は依頼文から予測できない」が効いていて、
  分量を問うと予測を求めることになる。段は予測ではなく指定である
- **CDK の `Secret` は既定でランダムな値を生成する。** 空の器のつもりで作ると出鱈目な鍵が入り、
  Jev を指したときに毎回 401 → 全件が竹に落ちて価格が実質固定になる。目印（`REPLACE_ME`）を
...
- 予算バケツ（決定57）は判定器を包む造りだったので、リクエストごとに選び直すと毎回満タンに戻り
...
## 2026-09-21: 価格判定器を TypeSafe の docs と突き合わせて点検する（決定53・56）

### やったこと

判定器の造りはそのままに、System One の設計原則（[docs](https://docs.typesafe.ai/)）と
突き合わせて `pricing/` を点検し、見つかった穴を塞いだ。Jev への差し替えはしていない。

- `judge`: 段の読み取りを完全一致から語単位の照合へ。`maxTokens` を 8 から 16 へ
...
- 見送ったもの: 人手へのエスカレーション（無認証の自動売買で人が介在しない）、
  依頼文→段の短期メモ化（`extra` は実測で往復しており、ステートレスを濁す割に得が薄い）、
...
- 評価セット（`eval-cases.test.ts`）は converse も fetch も差し替えており、ヘッダーが
  謳う「全段が同じ答えに潰れる事故を捕まえる」は実モデルに対しては果たせていない。
...
- `scoreToTier` が NaN で `Tier` を名乗る undefined を返していた（`TIER_ORDER[NaN]`）。
  ...
  有限でない値は中央の段へ収める
- `readTier` の正規表現が段の名前を直書きしており、同じ変更で `scoreToTier` を水準数から
  導くようにした意図と食い違っていた。`TIER_ORDER` から組み立てる
- 寛容化のテストが `fellBack` しか見ておらず、誤った段に読めても通る状態だった。
  入力と期待する段の組にして段そのものを検める
...
- **判定器と生成の Converse 共有**: 判定器の既定が生成用の `converse` を共有するため、
  偽 Converse を注入していた x402 往復テストが 6 件落ちた。「生成が呼ばれていないこと」の
  検査を、判定器の呼び出しが誤って満たしてしまう。テスト側で判定器を固定して解消したが、
...
- `maxTokens` は段の目安そのものではなく 3 倍を確保する。目安は引き寄せる力であって
...
- 判定に失敗したら竹（中央）に落とす。判定できないことで商いを止めるより害が小さく、
  誤りの向きを一方に固定しない
- 段の指示は元のシステムプロンプトを書き換えず別ブロックとして添える。段を渡さない
...
- 根拠の開示（決定56）を 402 応答に載せた。資源説明に「ご依頼の内容では、およそ 8000
  ...
  書かないことをテストで縛った
- U12 を決定57 へ昇格させ、判定器を呼び出し予算（バケツ）で包んだ。使い切ったら判定器を
  呼ばず既定の段で売る。止めると乱発する側の狙いどおりになるため、売り買いは止めない。
  依頼文は 2,000 文字で切り詰めて 1 回あたりの入力も抑える。依頼文の長さによる足切りも
  検討したが、実測では短い依頼が松に相当する例があり、長さと段が対応しないので採らなかった

### 文書と構成図（同日）

- README と billing-mcp/README を段階制の実態に合わせた。段の表、AppConfig からの
...
- `docs/architecture.drawio.png` に AppConfig と Secrets Manager を足し、Bedrock の説明を
  「HTML 生成（Sonnet）／段の判定（Haiku）」に改めた。埋め込み XML を取り出して編集し、
...
ところが決定56 では段が価格と生成規模の両方を決める。買い手が段を下げても、安くて
...
- `quote-seal.ts` は HMAC と引数の指紋と有効期限を落とし、版・段・価格を連ねただけの
...
付けた 4 段（small / medium / large / xlarge）。

| 判定器 | 規模 | 日本語 choice | 英語 choice | 英語 score | 速度 |
| --- | --- | --- | --- | --- | --- |
...
- **System One の枠組みは小型モデルほど有用**。同じ定義文を素の生成プロンプトで LFM に与えると
  6/20 まで落ち、20 件中 8 件は段の語すら取り出せなかった（criteria の鸚鵡返し、HTML を書き始める、
...
- **依頼文から出力量は予測できない**。実測ラベルに対する Haiku の的中は 10/20 で、最頻クラスだけを
  答える基準線 9/20 とほぼ同じ。20 件中 14 件を真ん中の段と答えており、判別できていない
...
予測できないなら決めてしまえばよい、というユーザーの提案で設計を変えた。段を判じたら、その段の
目安トークン数を生成時の指示に織り込む。
...
依頼に松を指示しても 9,000 台で頭打ちになるが、軽い依頼を松と判ずること自体が判定の誤りなので
実害はない（ユーザー判断）。

この転換により、判定器の仕事は「モデルが何トークン書くかを当てる」から「依頼に見合う規模を
判ずる」に変わる。後者は主観ラベルで測っていたものと同じであり、Haiku 19/20・ローカル 15〜18/20 と
いう数字が改めて意味を持ち直す。
...
- 段ごとの価格の決定（決定56）。実測原価は 梅 ≒ $0.075 / 竹 ≒ $0.12 / 松 ≒ $0.18。利幅は未決
- 段の判定器の選定。Haiku（19/20・1,037ms）とローカルの Qwen2.5-1.5B（15/20・239ms）のどちらを
