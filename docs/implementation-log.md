...
- 買い手への提示。「ご依頼の内容ではおよそ N トークン相当の分量になるため梅（$X）です」
  という根拠の開示（決定56）は、まだ 402 応答に載せていない
- U12（見積もり要求の乱発への防護）。402 を返す経路が Haiku の呼び出しを伴うように
  なったため、無認証の公開エンドポイントでは新たな負担の経路になる
- `upto` の併記（決定54 の割引と失敗時 0 決済）。U10 で署名できることは確かめたが、
  `@x402/mcp` に決済額の上書きが無いため自前の settle 経路が要る
- deploy しての実地確認。AppConfig の拡張レイヤー ARN を渡した経路は未検証
